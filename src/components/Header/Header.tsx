        ...
        <div className="fixed top-0 left-0 z-40 w-full bg-gray-0 flex gap-1 justify-between py-3 px-6 h-[90px]">
            ...
            <div className="flex items-center gap-6">
                <div className="flex items-center justify-center size-[66px] rounded-md">
                    ...
                    <Image src="/icons/filter-icon.svg" alt="Logout icon" width={25} height={22}
                           className="opacity-20 hover:opacity-100 duration-300 cursor-pointer"/>
                </div>
                    <CartButton/>
                    <div className="flex items-center justify-center size-[66px] rounded-md bg-gray-10" onClick={() => {
                        sessionStorage.removeItem('isLoggedIn')
                        push('/login')
                    }}>
                        <Image src="/icons/logout-icon.svg" alt="Logout icon" width={25} height={22}
                               className="opacity-20 hover:opacity-100 duration-300 cursor-pointer"/>
                    </div>
                </div>
            </div>
