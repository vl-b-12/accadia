...
const ProductInfo = ({ product }: { product: Product }) => (
  <div className="px-2 py-1.5 bg-violent-20 rounded-md">
    ...
    <KaratsBullet karats={product.karats} />
    <div className="flex flex-col gap-1.5">
      {product.characteristics.map((characteristic) => (
        <div
          key={characteristic.id}
          className="px-3 py-0.5 rounded-md text-xs bg-violent-10 font-medium text-gray-70 w-fit [&:last-child]:text-violent-40 [&:last-child]:bg-gray-0"
        >
...
const ProductFooter = ({
  ...
  const { currencySymbol, price } = formatPrice(product.price);
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="px-3 pt-4 pb-3 gap-6">
        ...
        <SkuBullet sku={product.sku} className="absolute z-30 top-4 left-4 " />
      </DialogContent>
    </Dialog>
  );
};
